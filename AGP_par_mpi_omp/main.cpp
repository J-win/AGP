...
int main(int argc, char *argv[]) {
  ...
  int omp_k = converter_in_number(argv[4]);
  double m = -1.0, mm = 0.0, minf = 0.0, minx = 0.0;
  int k = 0;
  int end = 1;
  double *smm = new double[5 * omp_k];
  double *tp = new double[2 * omp_k];
  ...
  if (ProcRank == 0) {
    ...
    std::priority_queue<interval> q;
    std::list<point>::iterator itl, itr;
    interval *zk = new interval[(ProcNum - 1) * omp_k];
    point* tt;

    double pr = (fun[j].b - fun[j].a) / ((ProcNum - 1) * omp_k);
    for (int i = 0; i < (ProcNum - 1) * omp_k; i++)
    ...
    do {
      ...
      for (int i = 0; i < ProcNum - 1; i++) {
		for (int v = 0; v < omp_k; v++) {
		  zk[i * omp_k + v] = q.top();
		  q.pop();
		  smm[5 * v] = zk[i * omp_k + v].lp->x;
		  smm[5 * v + 1] = zk[i * omp_k + v].lp->z;
		  smm[5 * v + 2] = zk[i * omp_k + v].rp->x;
		  smm[5 * v + 3] = zk[i * omp_k + v].rp->z;
		  smm[5 * v + 4] = m;
		  }
        MPI_Send(smm, 5 * omp_k, MPI_DOUBLE, i + 1, 0, MPI_COMM_WORLD);
	  }

	  for (int i = 0; i < ProcNum - 1; i++) {
        MPI_Recv(tp, 2 * omp_k, MPI_DOUBLE, i + 1, 1, MPI_COMM_WORLD, &status);
	  for (int b = 0; b < omp_k; b++) {
	    point t(tp[2 * b], tp[2 * b + 1]);
	    tt = insertup_list(&p, &t);
	    q.push(interval(Rfunc(*zk[i * omp_k + b].lp, *tt, m), zk[i * omp_k + b].lp, tt));
	    q.push(interval(Rfunc(*tt, *zk[i * omp_k + b].rp, m), tt, zk[i * omp_k + b].rp));
	    k++;
	  }
      }
      end = 1;
    ...
    do {
      MPI_Recv(smm, 5 * omp_k, MPI_DOUBLE, 0, 0, MPI_COMM_WORLD, &status);
      #pragma omp parallel num_threads(omp_k) shared(smm, tp)
      {
        int nt = omp_get_thread_num();
        tp[2 * nt] = 0.5 * (smm[5 * nt + 2] + smm[5 * nt]) - ((smm[5 * nt + 3] - smm[5 * nt + 1]) / (2.0 * smm[5 * nt + 4]));
        tp[2 * nt + 1] = f[j](tp[2 * nt]);
      }
      MPI_Send(tp, 2 * omp_k, MPI_DOUBLE, 0, 1, MPI_COMM_WORLD);
